...
lekerd.prototype.rendszamalapjan = function(req, res, callback){
    ...
    params = [rendszam],
    lekerdquery = 'SELECT auto.Gyarto, auto.Tipus, info.Evjarat, info.Allapot, info.Futottkm, info.VezetettSzervK, auto.Megbizhatosag, auto.Tipshiba, info.Okmanyok, info.Muszakierv, info.Alvazszam, info.Gumiabroncs, info.Kepcim, info.Torott FROM info INNER JOIN auto ON info.Auto_AID = auto.AID WHERE info.Rendszam = ?';
...
lekerd.prototype.alvazszamalapjan = function(req, res, callback){
    ...
    params2 = [alvazszam],
    lekerdquery = 'SELECT auto.Gyarto, auto.Tipus, info.Evjarat, info.Allapot, info.Futottkm, info.VezetettSzervK, auto.Megbizhatosag, auto.Tipshiba, info.Okmanyok, info.Muszakierv, info.Rendszam, info.Gumiabroncs, info.Kepcim, info.Torott FROM info INNER JOIN auto ON info.Auto_AID = auto.AID WHERE info.Alvazszam = ?';
    mysqlPool.getConnection(function(err, connection){
